...
async function getWeather(country) {
  try {
    let t = await fetch(
      `https://api.weatherapi.com/v1/forecast.json?key=03f2166c13fe419f926153727240612&q=${country}&days=3`
    );
...
function displayCurrent(data) {
  ...
  let container = `
      ...
      <div class="today-forecast w-25  ">
            <div class="forecast-header d-flex  justify-content-between">
              <h6 class="day">${new Date().toLocaleString("en-US", {
                weekday: "long",
              })}</h6>
              <h6 class="date">${new Date().toLocaleDateString("en-US", {
                day: "numeric",
                month: "long",
              })}</h6>
            </div>
            ...
            <div class="custom ms-4 mb-4 text-info my-2">${
...
function displayAnother(forecastday) {
  ...
  for (let i = 1; i < forecastday.length; i++)
    respones += `
      ...
      <div class="forecast2 w-25">
            <div class="forecast-header text-center text-white">
              <div class="day">${new Date(forecastday[i].date).toLocaleString(
                "en-US",
                { weekday: "long" }
              )}</div>
      ...
      <div class="custom text-info text-center mt-3">${
